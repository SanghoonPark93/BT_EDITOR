...
Play 모드에서 씬의 AI 오브젝트를 선택하고 **Window > Behavior Tree**를 엽니다. 창이 해당 AI의 트리 에셋과 키를 로드합니다. 현재 `RUNNING`인 가장 깊은 노드가 노란 테두리로 표시되며 그 위치로 스크롤합니다. 실행이 즉시 끝나는 트리는 마지막으로 실행된 노드를 성공(초록) 또는 실패(빨강) 상태로 표시합니다. 틱이 멈추면 잠시 후 강조 표시가 사라집니다. 사용자 정의 복합 노드에서 자식을 실행할 때는 `child.Tick()`을 호출해야 자식 실행 상태도 추적됩니다.
